...
function createApp(options = {}) {
    ...
    const todayStr =
        options.todayStr || (() => new Date().toISOString().slice(0, 10));
    const dbFile =
        options.dbFile ||
        process.env.PAKADLE_DB ||
        path.join(ROOT, "pakadle.db");
  ...
  `);
    ...
    try {
        db.exec("ALTER TABLE plays ADD COLUMN started_at TEXT");
    } catch {}
    // Password recovery. Accounts have no email, so self-service reset is
    // impossible. Instead an admin flips `recovery_mode` on an account via the
    // `pakadle` CLI; while set, the owner can enter just their name on the site's
    // "Forgot password?" screen and pick a new password. The flag clears on use.
    try {
        db.exec(
            "ALTER TABLE accounts ADD COLUMN recovery_mode INTEGER NOT NULL DEFAULT 0",
        );
    } catch {}
    ...
    if (!dailySeed) {
        const row = db
            .prepare("SELECT value FROM meta WHERE key = 'daily_seed'")
            .get();
        if (row) dailySeed = row.value;
        else {
            dailySeed = crypto.randomBytes(32).toString("hex");
            db.prepare(
                "INSERT INTO meta (key, value) VALUES ('daily_seed', ?)",
            ).run(dailySeed);
        }
    }
    const puzzleIdxFor =
    ...
    const PAKEIBA_ENABLED =
        process.env.PAKEIBA === "1" || process.env.PAKEIBA === "true";
    ...
    function puzzleForDate(dateStr) {
        ...
        if (!row) {
            ...
            db.prepare(
                "INSERT INTO puzzles (date, number, idx) VALUES (?, ?, ?)",
            ).run(dateStr, number, idx);
    ...
    function computeStats(pid) {
        const rows = db
            .prepare(
                "SELECT date, won, grid FROM plays WHERE pid = ? AND finished = 1 ORDER BY date",
            )
            .all(pid);
        ...
        for (const r of rows) {
            if (r.won) {
                run =
                    prev && dayNumber(r.date) - dayNumber(prev) === 1
                        ? run + 1
                        : 1;
    ...
    function getPlay(pid, date) {
        const row = db
            .prepare(
                "SELECT grid, finished, won, started_at FROM plays WHERE pid = ? AND date = ?",
            )
            .get(pid, date);
        return {
    ...
    function accountFromReq(req) {
        ...
        return (
            db
                .prepare(
                    "SELECT id, name, rating, wins, losses, draws FROM accounts WHERE id = ?",
                )
    ...
    function loadQaAccess() {
        try {
            const raw = JSON.parse(
                fs.readFileSync(
                    path.join(ROOT, "pakadb", "qa-access.json"),
                    "utf8",
                ),
            );
            if (!raw || !Array.isArray(raw.testers) || !raw.testers.length)
                return null;
            const m = new Map();
            for (const t of raw.testers)
                if (t && t.id)
                    m.set(String(t.id), {
                        salt: String(t.salt),
                        hash: String(t.hash),
            ...
            return null;
        } // missing/invalid file => gate off (open)
    }
    let qaTesters =
        process.env.PAKADB_QA_OPEN === "1" || options.qaOpen
            ? null
            : loadQaAccess();
    let qaSecret = options.qaSecret || process.env.PAKADB_QA_SECRET;
    if (!qaSecret) {
        const row = db
            .prepare("SELECT value FROM meta WHERE key = 'pakadb_qa_secret'")
            .get();
        if (row) qaSecret = row.value;
        else {
            qaSecret = crypto.randomBytes(32).toString("hex");
            db.prepare(
                "INSERT INTO meta (key, value) VALUES ('pakadb_qa_secret', ?)",
            ).run(qaSecret);
        }
    }
    function qaSign(id) {
        return crypto
            .createHmac("sha256", qaSecret)
            .update(String(id))
            .digest("hex");
    }
    ...
    function setQaCookie(req, res, id) {
        ...
        res.setHeader(
            "Set-Cookie",
            `pdqa=${val}; Path=/; HttpOnly; SameSite=Lax; Max-Age=1209600${secure}`,
        ); // 14 days
    }
    function clearQaCookie(req, res) {
        ...
        res.setHeader(
            "Set-Cookie",
            `pdqa=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0${secure}`,
        );
    }
  ...
  * { box-sizing: border-box; } body { margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center;
  ...
  <form class="card" method="POST" action="/api/pakadb/login" autocomplete="off">
    ...
    function applyMatch(aId, bId, result) {
        ...
        const get = (id) =>
            id
                ? db
                      .prepare(
                          "SELECT id, rating, wins, losses, draws FROM accounts WHERE id = ?",
                      )
                      .get(id)
                : null;
        const A = get(aId),
        ...
        const bump = (acc, col) =>
            db
                .prepare(`UPDATE accounts SET ${col} = ${col} + 1 WHERE id = ?`)
                .run(acc.id);
        if (A)
            bump(
                A,
                result === "a" ? "wins" : result === "b" ? "losses" : "draws",
            );
        if (B)
            bump(
                B,
                result === "b" ? "wins" : result === "a" ? "losses" : "draws",
            );
        if (A && B) {
            const K = 32;
            ...
            db.prepare("UPDATE accounts SET rating = ? WHERE id = ?").run(
                na,
                A.id,
            );
            db.prepare("UPDATE accounts SET rating = ? WHERE id = ?").run(
    ...
    function penalize(accountId, points) {
        ...
        const a = db
            .prepare("SELECT id, rating FROM accounts WHERE id = ?")
            .get(accountId);
        if (!a) return null;
    ...
    const server = http.createServer((req, res) => {
        ...
        if (
            ...
            url.pathname.startsWith("/pakapix/")
        ) {
            return pakapix.handle(req, res, url);
        }
        if (
            url.pathname === "/tailoftheday" ||
            url.pathname.startsWith("/tailoftheday/")
        ) {
        ...
        if (
            url.pathname === "/pakachess" ||
            url.pathname.startsWith("/pakachess/")
        ) {
        ...
        if (
            url.pathname === "/pakeiba" ||
            url.pathname.startsWith("/pakeiba/")
        ) {
            if (!PAKEIBA_ENABLED) {
                res.writeHead(404);
                return res.end("not found");
        ...
        if (url.pathname === "/pakadb" || url.pathname === "/pakadb/") {
            return fs.readFile(
                path.join(ROOT, "pakadb", "index.html"),
                (err, buf) => {
                    if (err) {
                        res.writeHead(404);
                        return res.end("not found");
        ...
        if (url.pathname === "/api/guess" && req.method === "POST") {
            ...
            return readBody(req, (body) => {
                ...
                if (play.finished)
                    return sendJson(res, { error: "already finished" }, 409);
                if (play.grid.length >= MAX_GUESSES)
        ...
        if (url.pathname === "/api/auth/register" && req.method === "POST") {
            return readBody(req, (body) => {
                ...
                if (
                    ...
                    return sendJson(
                        res,
                        { error: "Name must be 1–24 printable characters." },
                        400,
                    );
                }
        ...
        if (url.pathname === "/api/auth/login" && req.method === "POST") {
            return readBody(req, (body) => {
                ...
                const password = String(data.password || "");
                const acc = db
                    .prepare("SELECT * FROM accounts WHERE name_lower = ?")
                ...
                db.prepare(
                    "INSERT INTO sessions (token, account_id, created_at) VALUES (?, ?, ?)",
                ).run(token, acc.id, new Date().toISOString());
                setSessionCookie(req, res, token);
                return sendJson(res, { account: publicAccount(acc) });
            });
        }

        // Step 1 of "Forgot password?": given just a name, is this account allowed to
        // reset? Only if an admin flipped it into recovery mode via the `pakadle` CLI.
        // Unknown names return the same "not enabled" answer so this can't be used to
        // enumerate trainers.
        if (url.pathname === "/api/auth/reset-begin" && req.method === "POST") {
            return readBody(req, (body) => {
                ...
                const acc = db
                    .prepare(
                        "SELECT recovery_mode FROM accounts WHERE name_lower = ?",
                    )
                    .get(name.toLowerCase());
                return sendJson(res, { enabled: !!(acc && acc.recovery_mode) });
            });
        }

        // Step 2: set the new password. Allowed only while the account is in admin
        // recovery mode. On success the flag clears, all old sessions die, and the
        // owner is signed straight in on the new password.
        if (url.pathname === "/api/auth/reset" && req.method === "POST") {
            return readBody(req, (body) => {
                ...
                const acc = db
                    .prepare("SELECT * FROM accounts WHERE name_lower = ?")
                    .get(name.toLowerCase());
                if (!acc || !acc.recovery_mode) {
                    return sendJson(
                        res,
                        {
                            error: "Recovery isn't enabled for this account. Ask an administrator.",
                        },
                        401,
                    );
                }
                ...
                const hash = hashPassword(password, salt);
                db.prepare(
                    "UPDATE accounts SET salt = ?, hash = ?, recovery_mode = 0 WHERE id = ?",
                ).run(salt, hash, acc.id);
                ...
                const fresh = db
                    ...
                    .get(acc.id);
                return sendJson(res, { account: publicAccount(fresh) });
            });
        }
        ...
        if (url.pathname === "/api/auth/me" && req.method === "GET") {
            return sendJson(res, {
                account: publicAccount(accountFromReq(req)),
            });
        }
        ...
        if (url.pathname === "/api/leaderboard" && req.method === "GET") {
            const rows = db
                .prepare(
                    "SELECT name, rating, wins, losses, draws FROM accounts ORDER BY rating DESC, wins DESC, name ASC LIMIT 50",
                )
                .all();
        ...
        const media =
            /\.(png|jpe?g|webp|gif|svg|ico|woff2?|ttf|otf|mp4)$/i.test(ext);
    ...
    if (PAKEIBA_ENABLED)
        pakeibaOnline.init(server, pakachessWs, () => crypto.randomUUID());
...
if (require.main === module) {
    ...
    server.listen(PORT, () => {
        console.log(
            `Pakadle running → http://localhost:${PORT}  (${words.length} uma in the pool)`,
        );
    });
}
