...
(function () {
  ...
  css.textContent = [
    // account controls sit as one unit in the header, matching the nav pills
    ".acct-controls{display:inline-flex;align-items:center;gap:8px}",
    "#auth-btn{font-family:inherit;font-weight:700;font-size:.95rem;cursor:pointer;",
    "  height:38px;padding:0 18px;border:none;border-radius:999px;color:#fff;",
    "  background:var(--slate);box-shadow:0 3px 0 #625a72;",
    "  transition:transform .08s ease,box-shadow .08s ease}",
    "#auth-btn:active{transform:translateY(3px);box-shadow:0 0 0 #625a72}",
    "#acct-chip{display:inline-flex;align-items:center;gap:6px;height:38px;",
    "  font-weight:700;font-size:.95rem;color:var(--ink);background:var(--key-bg);",
    "  border:2px solid var(--tile-line);border-radius:999px;padding:0 14px;box-sizing:border-box}",
    "#acct-chip::before{content:'\\1F40E'}",
    "#acct-chip[hidden]{display:none}",
    "@media (max-width:560px){",
    "  #auth-btn{height:34px;padding:0 14px;font-size:.85rem}",
    "  #acct-chip{height:34px;padding:0 12px;font-size:.85rem}",
    "}",
